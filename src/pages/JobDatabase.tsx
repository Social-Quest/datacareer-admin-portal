...
const JobDatabase = () => {
  ...
  const handleRefresh = async () => {
    try {
      ...
      toast({
        title: "Success",
        description: response?.data?.message || "GCP Database refresh initiated successfully",
      });
      ...
      toast({
        title: "Error",
        description: error?.response?.data?.message || "Failed to refresh database",
        variant: "destructive",
      });
        ...
        <div className="data-card max-w-2xl">
          <div className="space-y-6">
            <div>
              <h3 className="text-lg font-semibold mb-2">Database Refresh</h3>
              <p className="text-sm text-gray-600 mb-4">
                Click the button below to initiate a refresh of the job database from GCP.
              </p>
            </div>
